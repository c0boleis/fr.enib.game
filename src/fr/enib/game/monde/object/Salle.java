...
import fr.enib.game.monde.core.Observateur;
import fr.enib.game.monde.core.Observe;
import fr.enib.game.monde.core.Situable;
import fr.enib.game.monde.core.Vec3;

/**
 * Classe implementant une salle du musee
 */
public class Salle extends Situable implements Observateur{
	...
	public static final String ARRIERE = "Arriere";
	public static final String GAUCHE = "Gauche";
	public static final String DROITE = "Droite";
	public static final String PLAFOND = "Plafond";
	public static final String SOL = "Sol";

	protected String textureMur;
	protected String textureSol;
	protected String texturePlafond;

	public CapteurCubique capteurPresenceAvatar;

	public int nbrPorte;
	public float hauteurPorte, largeurPorte;
	...
	public Salle(String id, float largeur, float profondeur, float hauteur){
		this(id, largeur, profondeur, hauteur, 2.0f, 3.5f);
	}
	 ...
	 */
	public Salle(String id, float largeur, float profondeur, float hauteur, float largeurPorte, float hauteurPorte){
		this(id) ;
		...
		ajouter( new Sol(SOL + getId() , this.textureSol, largeur, profondeur) ) ;
		ajouter( new Plafond(PLAFOND + getId(), this.texturePlafond, largeur, profondeur)) ;  
		ajouter( new Mur(GAUCHE + getId(), this.textureMur, hauteur, profondeur)) ; 
		ajouter( new Mur(ARRIERE + getId(), this.textureMur, hauteur, largeur)) ; 
		ajouter( new Mur(DROITE + getId() , this.textureMur, hauteur, profondeur)) ;
		ajouter( new Mur(AVANT + getId(), this.textureMur, hauteur, largeur) ) ; 
	 ...
	 */
	public boolean ajouterTableau(Tableau tableau, String mur) {
	 ...
	 */
	private float trouverPlaceTableau(float largeurTableau, String mur){
		Mur c = getCloison(mur);
	...
	public void ajouterTableau(Tableau t) {
		if(ajouterTableau(t, GAUCHE)) {
			//LOGGER.debug("Tableau " + t.getId() + "  ajouté !");
		}
		else if(ajouterTableau(t, DROITE)){
			//LOGGER.debug("Tableau " + t.getId() + "  ajouté !");
		}
		else if(ajouterTableau(t, ARRIERE)){
			//LOGGER.debug("Tableau " + t.getId() + "  ajouté !");
		}
		else if(ajouterTableau(t, AVANT)){
			//LOGGER.debug("Tableau " + t.getId() + "  ajouté !");
		}
	...
	public Plafond getPlafond(){
		return (Plafond) objets.get(PLAFOND+getId());
	}
	 ...
	 */
	public Sol getSol(){
		return (Sol) objets.get(SOL+getId());
	}
	...
	public Mur getMurAvant(){
		return (Mur) objets.get(AVANT+getId());
	}
	...
	public Mur getMurArriere(){
		return (Mur) objets.get(ARRIERE+getId());
	}
	...
	public Mur getMurDroite(){
		return (Mur) objets.get(DROITE+getId());
	}
	...
	public Mur getMurGauche(){
		return (Mur) objets.get(GAUCHE+getId());
	}
	...
	/**
	 * Revoie la texture (le chemin de l'image) des murs
	 * @return le chemin de l'image de la texture
	 */
	public String getTextureMur() {
		return textureMur;
	}
	...
	/**
	 * Revoie la texture (le chemin de l'image) du sol
	 * @return le chemin de l'image de la texture
	 */
	...
	/**
	 * Revoie la texture (le chemin de l'image) du plafond
	 * @return le chemin de l'image de la texture
	 */
	public String getTexturePlafond() {
		return texturePlafond;
	}
